...
class Player
  ...
  def initialize(player_type = :human) #allows differentiation btwn human and computer
  ...
  def set_name
    if human?
      ...
      self.name = ['R2D2', 'Han Tyumi', 'Hal', 'Chappie'].sample
    end
  end
...
class RPSGame
  ...
  def initialize
    @human = Player.new #don't have to set :human, because that is default in initialize
